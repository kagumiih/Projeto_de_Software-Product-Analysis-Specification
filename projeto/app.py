...
def seleciona_venda():
    if request.method == 'POST':
        venda_id = request.form['id_venda']
        venda = get_venda(venda_id)
        return render_template('seleciona_venda.html',venda=venda)
    else:
        return redirect(url_for('seleciona_venda'))  

@app.route('/venda/<int:id_venda>')
def venda_selecionado(id_venda):
    venda = get_venda(id_venda)
    return redirect(url_for('seleciona_venda', venda=venda))

def get_venda(id_venda):
    try:
        with pyodbc.connect(get_dados_conexao()) as cnx:
            cursor = cnx.cursor()
            cursor.execute("SELECT * FROM Venda WHERE id_venda=?", (id_venda,))
            venda = cursor.fetchone()
            cursor.execute("select usuario FROM Login WHERE id_login=?",(venda[1]))
            venda[1] = cursor.fetchone()
        return venda
    except pyodbc.Error as e:
        print("Erro ODBC:", e)
        return None
...
def seleciona_usuario():
    if request.method == 'POST':
        ...
        login = get_usuario(id_login)
        return render_template('seleciona_usuario.html',login=login)
    else:
        return redirect(url_for('seleciona_usuario'))  

@app.route('/usuario/<int:id_usuario>')
def usuario_selecionado(id_login):
    login = get_usuario(id_login)
    return redirect(url_for('seleciona_usuario', login=login))

def get_usuario(id_login):
    try:
        with pyodbc.connect(get_dados_conexao()) as cnx:
            cursor = cnx.cursor()
            cursor.execute("SELECT * FROM Login WHERE id_login=?", (id_login,))
...
@app.route('/excluir_produto', methods=['POST', 'GET'])
def excluir_produto():
    if request.method == 'POST':
        product_id = request.form['id_produto']
